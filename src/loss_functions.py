...
class SoftLossTwoState(tf.keras.losses.Loss):
    """
        The loss function consists of two parts. The first part is the mean squared error between the true and the predicted values.
        The second part is the mean squared error between expected domain knowledge parameters and the absolute differences
        between the differences of the true values and the differences of the predicted values.
    ...
    def __init__(self, theta, steepness, tolerance=0.1, normalized=False, name="loss_two_state"):
        super().__init__(name=name)
        self.theta = theta
        self.steepness = steepness
        self.normalized = normalized
    ...
    def call(self, y_true, y_pred):
        ...
        tgds_loss_trans = criterion(out_of_range_low + out_of_range_high, tf.zeros_like(y_difference))
        ...
        tgds_loss_static = criterion(out_of_range_high, tf.zeros_like(y_difference))
...
class WeightedLossTwoState(tf.keras.losses.Loss):
    """
        The loss function consists of two parts. The first part is the mean squared error between the true and the predicted values.
        The second part is the mean squared error between expected domain knowledge parameters and the absolute differences
        between the differences of the true values and the differences of the predicted values.
...
class LossTwoStateDiffRange(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        tgds_loss_trans = criterion(out_of_range_diff_low + out_of_range_diff_high, tf.zeros_like(y_difference))
